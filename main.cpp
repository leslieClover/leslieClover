...
private:
    ...
    public:
        ...
        void Print_Board(uint8_t Board[15][15]) {
            for (int i = 0; i < 15; ++i) {
                for(int j = 0; j < 15; ++j) {
                    cout << Board[i][j] << ' ';
                }
...
public:
    ...
    uint8_t game() {
        ...
        // cout << "272(" << (uint32_t)nodeRoot->cntX << ',' << (uint32_t)nodeRoot->cntY << ")" << endl;
        //
        // for (auto& i : openTable) {
        //     cout << "'275('" << (uint32_t)i->cntX << ',' << (uint32_t)i->cntY << ")" << endl;
        //
        // }
        int ct = 0;
        while (!openTable.empty()) {
            ...
            // cout << "282(" << (uint32_t)node->cntX << ',' << (uint32_t)node->cntY << ")" << endl;
            ...
            if (node->depth < maxDepth) {
                ...
                // cout << "numExpand is : " << (uint32_t)numExpand << endl;
                // for (auto& child : node->children) {
                //     cout << "node .child is : " <<" (" << (uint32_t)node->cntX << ',' << (uint32_t)node->cntY << ")" << endl;
                // }
                if (numExpand != 0) {
                    // cout << "numExpand != 0" << endl;
                    continue;
                }
            }
            node->evaluate();
        ...
        for (int i = 0; i < 15; i++) {
            ...
            for (int j = 0; j < 15; j++) {
                if (nodeNext->board[i][j] == 'B') {
                    if (reverse) cout << "W "; else cout << "B ";
                }else if (nodeNext->board[i][j] == 'W') {
                    if (reverse) cout << "B "; else cout << "W ";
                }else {
                    cout << "- ";
                }
